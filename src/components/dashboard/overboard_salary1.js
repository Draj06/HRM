...
import {SALARY_GRAPG1_OVERBOARD_ON_CLICK_CHANGE,SALARY_GRAPH1_2_DEPT_WISE_POPUP} from '../../queries'
import Modal from 'react-modal'
import { useHistory } from "react-router-dom";

const Overboard_salary1 =()=>  {
...
const {error,loading,data} = useQuery(SALARY_GRAPG1_OVERBOARD_ON_CLICK_CHANGE, {
...
const modalClick = (e) => {
 ...
 const empNameOnClick=(emp)=>{
  ...
  const { current_mon, previous_mon } = data.getDynamicSalaryOverBoardByMonthYear;

  let variationCurr = current_mon.cvariation
  let variationPrev = previous_mon.pvariation
  let currGross = current_mon.cgross
  let currCTC = current_mon.cctc
  let prevGross = previous_mon.pgross
  let prevCTC = previous_mon.pctc
  
  let Percentage = ((currGross-currCTC)/currGross)*100 || 0
  ...
  {
    minusVarCur = numDifferentiation(current_mon.cvariation)
  }
  if(variationPrev<0)
  {
  ...
  {
    minusVarPrev = numDifferentiation(previous_mon.pvariation)
  }
  ...
  return (
     ...
     <div className="row widthContainer">
       <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
         <MonthData
           ctc={numDifferentiation(previous_mon.pctc)}
           gross={numDifferentiation(previous_mon.pgross)}
           ctcLabel="CTC"
       ...
       <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
         <MonthData
           ctc={numDifferentiation(current_mon.cctc)}
           ctcLabel="CTC"
           gross={numDifferentiation(current_mon.cgross)}
           variation={minusVarCur}
           variationLabel="Var"
           month={numDifferentiation(current_mon.cmon_year)}
...
else
{
        ...
        <div className="row widthContainer">
          <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
            <MonthData
              ctc={numDifferentiation(previous_mon.ctc)}
              gross={numDifferentiation(previous_mon.gross)}
              ctcLabel="CTC"
              variation={minusVarPrev}
              variationLabel="Var"
              month={previous_mon.mon_year}
              percent={Percentage.toFixed(2)}
            />
          </div>
          <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
            <MonthData
              ctc={numDifferentiation(current_mon.ctc)}
              ctcLabel="CTC"
              gross={numDifferentiation(current_mon.gross)}
              variation={minusVarCur}
              variationLabel="Var"
              month={numDifferentiation(current_mon.mon_year)}
              percent={Percentage.toFixed(2)}
            />
          </div>
        </div>
...
export default Overboard_salary1